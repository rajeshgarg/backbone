        ...
        return (function () {
            ...
            var Field = Backbone.View.extend({
                ...
                render:function () {
                    var schema = this.schema,
                        el = $(this.el);
                    el.addClass('bbf-field' + schema.type).addClass("field"+_.dasherize(schema.title));

                    if (schema.cssClass) el.addClass(schema.cssClass); //note: tibbr modification
                    ...
                    el.html(Field.template({
                        ...
                        id:editor.id,
                        type:schema.type,
                        required:schema.required ? "<em>*</em>" : "" //note: tibbr modification
                    }));

                    //Add the editor
                    var $editor = $(editor.render().el);   //note: tibbr modification
                    if (schema.required) {                //note: tibbr modification
                        $editor.addClass("required");  //note: tibbr modification
                    }
                    $('.bbf-editor', el).html($editor);
            ...
            editors.Base = Backbone.View.extend({
                ...
                initialize:function (options) {
                    ...
                    if (options.model) {
                        ...
                        var modelType = this.model.get('modelType').toLowerCase();
                        if (_.isBlank(modelType)) modelType = "form";
                        this.model = options.model;
                        this.key = options.key;
                        this.nameKey = this.nameKey || modelType + '[' + this.key + ']';
                    ...
                    $(this.el).attr({"name":(this.nameKey || this.key), "rel":this.key, "disabled":this.schema.disabled, "multiple":this.schema.multiValue}); //note: tibbr modification
            ...
            editors.Radio = editors.Select.extend({
                ...
                _arrayToHtml:function (array) {
                    ...
                    _.each(array, function (option, index) {
                        ...
                        if (_.isObject(option)) {
                            var val = option.val ? option.val : '';
                            itemHtml += ('<input type="radio" name="' + self.nameKey + '" value="' + val + '" id="' + self.id + '-' + index + '"  rel="' + self.id + '" />') //tibbr modification self.namekey and ref
            ...
            editors.Checkboxes = editors.Select.extend({
                ...
                _arrayToHtml:function (array) {
                    ...
                    _.each(array, function (option, index) {
                        ...
                        if (_.isObject(option)) {
                            var val = option.val ? option.val : '';
                            itemHtml += ('<input type="checkbox" name="' + self.nameKey + '" value="' + val + '" id="' + self.id + '-' + index + '"  rel="' + self.id + '"  />')
                            itemHtml += ('<label for="' + self.id + '-' + index + '">' + option.label + '</label>')
                        }
                        else {
